...
- **User Persona**: 用户是资深的自动化软硬件设计与真空镀膜机构设计专家。请始终保持高级别的技术深度，在涉及软硬件结合、底层交互、工控通信或系统底层优化时，提供符合工业级标准的方案。
...
- **Terminal Constraints**: 严禁在未经询问的情况下执行具有破坏性的终端命令（如 `rm -rf`, 数据库重置等）。执行前必须展示完整命令并说明原因。
