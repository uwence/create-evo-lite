...
adopted spec 由 `mem verify` 的 `📋 [Spec Portfolio]` 段持续报账；老化（超 agingDays 无活动）或超标会常驻 ⚠️，直到 `mem spec park|reactivate` 表态（`park` 支持 `--until`）。park 会级联冻结其 plan。查任意 spec 当前状态用 `mem spec status`，均支持 `--json`。
