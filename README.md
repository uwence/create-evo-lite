...
> - **外部集成边界**：context-mode、RTK、GitNexus、独立 MCP 配置等都属于可选外部能力，不由 Evo-Lite 统一脚手架分发或接管；若项目需要它们，应由各自的安装器与配置体系管理。
...
> 升级旧项目后，建议先运行 `node .evo-lite/cli/memory.js verify` 检查 CLI、本地索引、状态文件与历史记忆是否处于可继续接管的状态。
...
* **🌐 In-Tree RAG (纯本地 FTS 引擎)**
  底层使用 `SQLite FTS5 + trigram + BM25`，索引和数据库都跟项目一起待在 `.evo-lite/` 下。想查历史 bug、架构决策或绕坑经验，不需要外部服务。
* **🧠 轻量本地检索**
  - **召回**：FTS5/trigram 负责从本地记忆里捞候选。
  - **排序**：BM25 负责把更接近查询意图的片段排到前面。
  这条链路不依赖外部模型下载，也不要求本地 ONNX 推理环境。
* **🛡️ 显隐双层记忆**
  - **显性状态机（`active_context.md`）**：焦点、backlog、trajectory 都有锚点边界，适合接管与交接。
  - **隐性记忆库（`memory.db` / `raw_memory` / `index_memory`）**：结构化归档与本地索引并存，支持后续重建与同步。较早的仓库里如果仍看到 `vect_memory/`，那是 `index_memory/` 的兼容旧命名；当前运行时会在条件允许时把它自动迁到 `index_memory/`。
* **🛠️ 可重建的归档管线**
  `archive`、`sync`、`rebuild` 让记忆不是“一次性写进去就听天由命”，而是可以补齐索引、整体重建、跨项目迁移。
...
> 不要停留在另一个仓库或无关目录里，直接用绝对路径调用目标项目中的 `./.evo-lite/mem` 或 `\.\.evo-lite\mem.cmd`；否则 `remember`、`archive`、`context track` 这类带 Git 溯源的记录，可能会继承**调用方当前目录**的 commit / git 状态，而不是目标项目本身。

> [!TIP]
> **内嵌本地检索引擎**：
> - 索引层：`SQLite FTS5 + trigram`
> - 排序层：`BM25`
...
```
这一步会检查本地索引、记忆库、`active_context.md`、离线记忆残留以及当前工作区的可接管性。
...
当 recall-first takeover 生效时，首屏还应该额外告诉你：它查了哪些历史锚点、是否命中、哪些命中改变了检查顺序；若没命中，则应明确说“按 fresh takeover 处理”。
...
└── .evo-lite/                    <-- (记忆存储与依赖区)
    ...
    └── index_memory/             - 已建立索引的档案标记
```

  > [!NOTE]
  > 较早的 dogfood 仓库可能仍然使用 `.evo-lite/vect_memory/`。当前运行时在发现 `vect_memory/` 存在且还没有 `.evo-lite/index_memory/` 时，会优先把旧目录自动迁移到新目录；因此同一个项目通常只会看到 `index_memory/` 或 `vect_memory/` 其中之一，而不会两者同时并存。
...
3. **记忆要可重建，不只是可写入**：长期可维护的记忆系统，关键不在“先记住一次”，而在“以后还能迁移、补索引、重排、校验、升级”。
...
- **写回方式**：使用 `peter-evans/create-pull-request` 反向开 PR，把生成的 `.evo-lite/raw_memory/**` 与 `.evo-lite/index_memory/**` 提交回主分支评审；较老项目若仍使用 `.evo-lite/vect_memory/**`，workflow 也会一并兼容，避免 protected branch 冲突。
...
- **Archive 列表** — 列出 `raw_memory/` 中每个文件，并标注是否已生成 `index_memory/` 标记（较老项目可能显示为 `vect_memory/`，pending / ok）。
- **索引空间分布** — 列出每个 namespace（`prose` / `code` / `symbol`）的本地引擎标识、维度、chunk 数。
