...
AI (无论何时，你或你的 AI 智能体都可以直接命令大脑查询或写入：
```bash
./evo recall "上一次登录接口调不通是因为什么？"
```

当完成了一段极其坎坷的重构，可以将经验值（血泪）永久化入向量库：
```bash
./evo remember "处理 Axios 的 502 错误是因为系统代理干扰，最终通过增加 proxy:false 解决"
```
