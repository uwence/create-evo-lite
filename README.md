...
* **🛡️ 显隐双层记忆**
  ...
  - **隐性记忆库（`memory.db` / `raw_memory` / `index_memory`）**：结构化归档与本地索引并存，支持后续重建与同步。
* **🛠️ 可重建的归档管线**
...
- **写回方式**：使用 `peter-evans/create-pull-request` 反向开 PR，把生成的 `.evo-lite/raw_memory/**` 与 `.evo-lite/index_memory/**` 提交回主分支评审，避免 protected branch 冲突。
...
- **Archive 列表** — 列出 `raw_memory/` 中每个文件，并标注是否已生成 `index_memory/` 标记（pending / ok）。
