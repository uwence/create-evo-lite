...
> [!IMPORTANT]
> **版本重大更新警示 (v1.5.3)**：本项目经历了向量维度的架构跃迁。现已全面实装 **Interactive Vectorize Pipeline (交互式升维管线)** 与 **Raw Memory 1:N 语义隔离引擎**。由于涉及到彻底的模型剥离和数据库指纹重置，新版本会自动在重建脑区前为您生成带时间戳的安全备份 `.bak`。建议务必在 Antigravity 等 Agent 终端中阅读最新文档后再深入使用。
