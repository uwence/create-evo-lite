...
- 日期:2026-07-23
- 状态:待复阅(设计 §1/§2/§3 已逐节口头批准;外部复阅 P0 已折入)
...
readOwner(dir)
//   解析成功返回对象;文件不存在或 JSON 损坏返回 null(损坏 ≠ 可清理,见四道闸)。
...
| ② 身份 | snapshot.isNode **且** commandLine 含 owner.entrypoint(`memory.js`)**且**(owner.processStartedAt 存在时)snapshot.startedAt 与之吻合(±2s 容差,格式归一后比对) | 任一不符 → `unknown`(PID 已被复用或非本系进程,绝不杀) |
...
前置条件:`readOwner` 返回 null(缺失/损坏)或 snapshot 关键字段为 null(查询失败/权限不足)→ 直接 `unknown`,不进闸。
...
└─ 锁错误 → backoff 重试 3×100ms(吸收 CLI/MCP 瞬时交错)
    ...
    └─ 仍失败 → diagnoseLockConflict
        ...
        ├─ 死持有者(闸① ESRCH)→ 清 stale owner → 最终重试一次
...
6. 已消失 → 删除死者 owner.json(此处不走 CAS —— 死者无法自清;
   四道闸 + 死亡确认即删除授权)→ 最终重试 openFn 一次。
...
- 启动即设 `process.env.EVO_LITE_INDEX_EPHEMERAL = '1'`(MCP 进程内生效)。
- `process.stdin.on('end', shutdown)` + `process.stdin.on('close', shutdown)` +
  transport/server `onclose → shutdown`。
- `shutdown()`:幂等(一次性标志)→ 关闭 memory index(`close()` →
  finalize + clearOwner)→ `process.exit(0)`。
...
- `mode: 'mcp' | 'cli'`(进程角色);`access: 'write'`(0.5.0 全部为写;
  `'read'` 预留给 `[zvec-06-upgrade]`)。
...
- 原子写:owner.json 任一时刻可完整 JSON.parse(写入路径走 tmp+rename 的实现断言)。
...
### T-lock-orphan-refusal-matrix(复阅 P0-1;安全否决面)
