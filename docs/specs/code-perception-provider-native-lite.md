...
Selection 分两阶段:**inspect(async)** await 每个已注册 Provider 的 `check()`+`getStatus()`,把抛错 Provider 隔离成 diagnostic 候选;**select(纯同步,exhaustive-testable)** 对候选决策:(1) 精确 preferredProvider 且 ready+具备能力 → 优先(不 ready/不具备且 `allowFallback=false` → 显式 `candidate:null`)→ (2) 排除**不 ready 或不具备所请求能力**的候选(而非笼统排除 unindexed)→ (3) 结构能力下 structural-primary 先于 enrichment,再按 freshness(fresh > unknown > stale)→ (4) 无结构候选且 fallback 支持该能力 → 用 Native Lite(degraded)→ (5) 无任何 ready 候选具备该能力 → 显式 degradation diagnostics(**ready-centric**,不回落 index-centric 语义)。
...
数据源: `git ls-files`(`-z` NUL 枚举,tracked + eligible untracked,经 `--exclude-standard` 覆盖 `.gitignore`)、Architecture IR、Planning IR、Git diff、file content hashes(sha256)。(`.evoignore` 本 MVP 不实现,见 §12 Follow-ups。)
...
- **Fake provider** `provider:fixture`(测试资产,`test/fixtures/code-perception/fixture-provider.js`,不进生产 registry): 测 capability routing、failure isolation、freshness。
...
├── provider-contract.js     # 契约 + validateProvider/Availability/Status
├── normalize.js             # CodeReference / result 归一化
├── native-lite.js           # provider:native-lite
├── provider-loader.js       # 加载/隔离 provider,失败不全局崩(allowlist-only)
└── provider-router.js       # 两阶段 capability routing + freshness
templates/cli/test/fixtures/code-perception/
├── fixture-provider.js      # 测试假 provider(id provider:fixture,不进生产 registry)
├── fixture-status.json
├── fixture-query.json
├── fixture-callers.json
└── fixture-impact.json
```
Runtime mirror `.evo-lite/cli/code-perception*` 与 `.evo-lite/cli/test/fixtures/code-perception*` 必须 byte-identical(fixture 经 manifest 登记以便镜像;安全边界靠 `DEFAULT_REGISTRY` 仅含 native-lite,而非 manifest 排除)。
...
## 12. Follow-ups(移出本 MVP)

- `.evoignore` 语法与 matcher:本 MVP 用 `git ls-files --exclude-standard` 覆盖 `.gitignore`;`.evoignore` 待统一 matcher 设计后单独 spec/plan(与 Plan Follow-ups 对齐)。
- source 大文件流式/分块读取:本 MVP `getEntity` 单次 `MAX_FILE_BYTES` 上限即可。
