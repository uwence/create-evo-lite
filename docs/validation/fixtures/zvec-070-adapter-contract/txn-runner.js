...
for (const phase of ['A', 'B', 'C']) {
    ...
    const status = timedOut ? 'timeout'
        : (!ev ? 'process-death'
            : (ev.error ? 'error' : (ev.allPassed ? 'passed' : 'failed')));

    record.phases.push({
        phase, status, exitCode: res.status, signal: res.signal,
...
const seamProven = record.phases.every(p => Array.isArray(p.seam) && p.seam.length > 0
    && p.seam.every(s => s.redirectedTo === ZVEC_ENTRY));
const blobOk = record.subject.blobMatchesExpected !== false
...
record.verdict = (allPhases && seamProven && blobOk
