...
### Session 10 — 2026-07-22 — 主用户五问(sprint 收束数据)

- **五问原答(主用户):**
  1. 更容易知道项目当前状态? —— **并没有,我不知道在哪里看当前状态**
  2. 知道 Agent 修改了哪个架构层? —— **并没有,我不知道在哪里看当前状态**
  3. 能快速判断 Agent 是否偏离目标? —— **并没有,我不知道在哪里看当前状态**
  4. 更换 Agent 后需要重新解释大量上下文? —— **不需要**
  5. 希望有可点击、可浏览的项目全貌? —— **是的需要**;哪怕非同步、刷新一次临时生成也行;要能直观看到"当前在操作架构中的哪个模块 + 各模块进展";最好图文并茂(架构图 + 模块文档,点击模块见其进展与状态)
- **P9(终局发现,全 sprint 最大反转):人侧入口缺口** —— 9 个 Session 里所有 Agent 都找到了治理面(focus 逐字、行号级定位、GitHub-only 重建全貌),但**主用户本人不知道看哪里**。治理面 100% 服务了 Agent,0% 触达了人。而入口其实存在:`mem inspect` 已有 timeline/planning/architecture/drift/verify 的浏览器页面、`mem bootstrap` 已有接管报告、`mem verify` 已有健康摘要 —— 缺的不是能力,是**人的已知入口**。P2(Agent 可发现性)与 P9(人的可发现性)同根:好能力不自我宣告。
- **决策规则应用:**
  - Q4=否 → 情况一的"Agent 侧"半边成立(P8 佐证):**跨 Agent 交接无需重复解释,4a+治理面已兑现核心承诺**。
  - Q1-Q3=失败 → 非"命令输出不直观"(情况二原义),而是入口未知 → **4a.x-H(人侧)**:让 `mem inspect`/`bootstrap` 被知道、被打开,而非新造能力。
  - Q5=明确需要 → **构成 4b-1 激活标准的第一次记录在案的真实需求**。但硬标准要求"≥2 次 + CLI/MCP 无法有效替代":主用户尚未用过已存在的 `mem inspect` —— 无法替代未被证明。**故本次裁定:不立即激活 4b-1**;先让主用户在下一次真实开发中实际使用 `mem inspect`,若仍明确需要点击式架构图+模块进展(第二次记录),则激活 4b-1,且范围按主用户描述重定义:**架构图为骨、模块进展/状态为肉的点击视图**(建 于 architecture IR + dashboard-data + unified explore,而非 code-symbol wiki)。
- **Sprint 终局裁定(依决策规则):**
  1. **4a:足够且已兑现**(Agent 侧全绿;P5/P7 界定其真实生态位=治理上下文面)
  2. **启动一轮小 4a.x**,含三项:a) 人侧入口(P9):README/AGENTS.md 顶部显著给出 `mem inspect`/`bootstrap` 三命令;b) Agent 裸指令引导(P2):接手聚合命令或 rules 显式路由;c) P4 时点归因语义小修(可选)
  3. **4b-1:strike 1/2 已记录,暂不激活**;主用户实测 `mem inspect` 后复裁
  4. **Evidence IR:继续 parked**(9b 显示治理链已有真实效用,结构链无人索取)
  5. **CodeGraph 子项目采纳:可选实验**(P7 的兑现路径,由主用户按需)

### Session 11 — 2026-07-22 — 主用户实测 `mem inspect`:不满足(strike 2/2,4b-1 激活)

- **实测评价(主用户原话要旨):**"从一个专业的文字面板(与 AI 交互的 CHAT 界面)进入另一个各种专业用语的面板。"
  - Active context 页:还算直观 ✅
  - Planning 页:看着费劲(术语密度)❌
  - Archive 页:没啥意义 —— 只见 memory 文件名且**点不开** ❌
  - 其余页面类似 ❌
  - 参照系:**Google Code Wiki** —— "看下它就知道我们和它之间的差异"
- **裁定:硬标准两条件齐备** —— ① 主用户在真实使用中第二次明确需要可浏览视图(Session 10 Q5 = strike 1,本次 = strike 2);② CLI/MCP + 现有 inspector **实测无法替代**(不是没入口,是入口的内容形态不对:面向治理术语,不面向人)。**依决策规则,4b-1 正式激活。**
- **范围重定义(取代原 cw-wiki 的 code-symbol wiki 设想):**
  - 骨:架构图(模块为节点,可点击)
  - 肉:每模块一页 —— 人话描述 + 进展/状态(plan/task 进度投影)+ 关键文档链接
  - 刷新模型:**按需重新生成即可**(主用户明确接受非同步)
  - 语言形态:人读的叙述文本,不是治理 IR 术语的换皮
