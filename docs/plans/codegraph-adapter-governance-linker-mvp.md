...
- **Derived outputs (deterministic sort + atomic write, git-tracked derived data):** `.evo-lite/generated/code-perception/governance-links.json` (the §3.4 stored graph), `.evo-lite/generated/code-perception/post-commit-last-run.json`.
...
- [ ] [task:cg-dogfood-validate] dogfood-validate.js + `--require-live-codegraph` strict flag (fingerprint recompute)
  ...
  - verify: node templates/cli/test.js governance
  - acceptance: `validateDogfoodArtifact(text,{requireClosureEvidence})` valid:true for dogfood-sample.md (metadata + RECOMPUTED matching fingerprints + all sections), valid:false w/ specific findings for dogfood-bad.md (missing section), a fingerprint-tampered copy (recompute mismatch), and a missing closureEvidenceCommit; `node templates/cli/test.js governance --require-live-codegraph` → exit 1 when `docs/code-perception-codegraph-dogfood.md` absent/invalid, exit 0 only on a valid real artifact; normal `governance` (no flag) does NOT require the artifact; command keeps `node ./.evo-lite/cli/test.js` prefix
  - test-first: governance.js「T-cg-dogfood-validate」runs validator on the two samples + tampered + no-closure-evidence; a subtest invokes the strict flag against a missing artifact and asserts non-zero exit (spawn `node templates/cli/test.js governance --require-live-codegraph` in a temp cwd, expect exit 1); 红→绿
